...
class WaterSampleLib
  ...
  # with your assignment as a MySQL dump
    ...
    def find(sample_id)    
      sample = WaterSample.find(sample_id)
      @@id = sample.id
      @@site = sample.site
      @@chloroform = sample.chloroform
      @@bromoform = sample.bromoform
      @@bromodichloromethane = sample.bromodichloromethane
      @@dibromichloromethane = sample.dibromichloromethane
    ...
    def bromodichloromethane()
      @@bromodichloromethane
    end

    def dibromichloromethane()
      @@bromodichloromethane
    end
    ...
    def factor(factor_weights_id)
      ...
      if @@id != nil
        ...
        chloroform_total = factor_weight.chloroform_weight * @@chloroform
        bromoform_total = factor_weight.bromoform_weight * @@bromoform
        bromodichloromethane_total = factor_weight.bromodichloromethane_weight * @@bromodichloromethane
        dibromichloromethane_total = factor_weight.dibromichloromethane_weight * @@dibromichloromethane
        total_factor = (chloroform_total + bromoform_total + bromodichloromethane_total + dibromichloromethane_total)
      
      end  
      return total_factor.round(5)
    ...
    def to_hash(include_factors = false)
      ...
      hashed[:id] = @@id
      hashed[:site] = @@site
      hashed[:chloroform] = @@chloroform
      hashed[:bromoform] = @@bromoform
      hashed[:bromodichloromethane] = @@bromodichloromethane
      hashed[:dibromichloromethane] = @@dibromichloromethane
      if include_factors == true
        all = self.get_all_factors()
        all.each do |i|
          hashed[":factor_#{i}"] = self.factor(i).round(5)
        end
      end
      return hashed
